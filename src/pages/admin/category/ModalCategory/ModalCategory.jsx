...
import { addCategory, putCategory } from '~/features/category/categoriesSlice';
const validationSchema = Yup.object({
  name: Yup.string().required('Vui lòng nhập tên loại'),
  description: Yup.string().required('Vui lòng nhập miêu tả'),
  avatarUrl: Yup.string().required('Vui lòng nhập đường dẫn hình ảnh'),
});
const ModalCategory = () => {
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        if (type === 'add') {
          ...
          await dispatch(putCategory(updatedValues));
          resetForm();
        }
      } catch (error) {
